  ...
  return (
    <div className="py-12 sm:py-16">
      <PageHeader
        ...
        title="Methodology"
        lede="GAFFER is a pragmatic reimplementation of the kind of national-team model Michael Caley builds as PADDLIN'. Six steps take raw results all the way to title odds."
      />

      <div className="mt-12 flex flex-col gap-px overflow-hidden rounded-xl border hairline">
        {STEPS.map((s) => (
          <div
            key={s.n}
            className="grid grid-cols-1 gap-4 bg-[var(--color-bg-subtle)] p-6 sm:grid-cols-[auto_1fr] sm:gap-8 sm:p-8"
          >
            <div className="display mono text-3xl text-[var(--color-accent)] sm:text-4xl">
              {s.n}
            </div>
            <div>
      ...
      <div className="panel mt-10 p-7">
        ...
        <p className="mt-3 max-w-3xl text-base text-[var(--color-text-secondary)]">
          The approach is lifted, with gratitude, from Michael Caley and Mike Goodman&apos;s{" "}
          <a
            className="text-[var(--color-accent)] hover:underline"
            href="https://www.youtube.com/@DoublePivotPod"
            target="_blank"
            rel="noreferrer"
          >
            Double Pivot
          ...
          <a
            className="text-[var(--color-accent)] hover:underline"
            href="https://www.expectinggoals.com"
            target="_blank"
            rel="noreferrer"
          >
            Expecting Goals
          ...
          of Caley&apos;s full model still missing is expected-goals (xG / xElo) data, which is hard
          to source for all international teams. It is a forecast, not a guarantee — football is
          gloriously random.
        </p>
        <div className="mt-5 flex flex-wrap gap-2">
          <Chip>model v{meta.modelVersion}</Chip>
          <Chip>{meta.nSims.toLocaleString("en-US")} sims</Chip>
          <Chip>data through {meta.dataThrough}</Chip>
          <Chip>home edge ×{meta.homeAdv.toFixed(2)}</Chip>
        </div>
      </div>
    </div>
  );
